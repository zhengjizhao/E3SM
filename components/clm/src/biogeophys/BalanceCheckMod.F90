  ...
  subroutine BeginColWaterBalance(bounds, &
   ...
   subroutine ColWaterBalanceCheck( bounds, num_do_smb_c, filter_do_smb_c, &
     ...
     associate(                                                                         &
       ...
       if ( found ) then
           print *, 'WARNING:  water balance error ',&
                ' nstep= ',nstep, &
                ' local indexc= ',indexc,&
          ...
          if ((col_pp%itype(indexc) == icol_roof .or. &
             ...
             print *, 'clm urban model is stopping - error is greater than 1e-4 (mm)'
             print *, 'nstep                      = ',nstep
             print *, 'errh2o                     = ',errh2o(indexc)
             print *, 'forc_rain                  = ',forc_rain_col(indexc)
             print *, 'forc_snow                  = ',forc_snow_col(indexc)
             print *, 'endwb                      = ',endwb(indexc)
             print *, 'begwb                      = ',begwb(indexc)
             print *, 'qflx_evap_tot              = ',qflx_evap_tot(indexc)
             print *, 'qflx_irrig                 = ',qflx_irrig(indexc)
             print *, 'qflx_supply                = ',atm2lnd_vars%supply_grc(g)
             print *, 'f_grd                      = ',ldomain%f_grd(g)
             print *, 'qflx_surf                  = ',qflx_surf(indexc)
             print *, 'qflx_qrgwl                 = ',qflx_qrgwl(indexc)
             print *, 'qflx_drain                 = ',qflx_drain(indexc)
             print *, 'qflx_snwcp_ice             = ',qflx_snwcp_ice(indexc)
             print *, 'qflx_lateral               = ',qflx_lateral(indexc)
             print *, 'total_plant_stored_h2o_col = ',total_plant_stored_h2o_col(indexc)
             print *, 'clm model is stopping'
             stop
          else if (abs(errh2o(indexc)) > 1.e-4_r8 .and. (nstep > 2) ) then

             print *,'clm model is stopping - error is greater than 1e-4 (mm)'
             print *,'colum number               = ',col_pp%gridcell(indexc)
             print *,'nstep                      = ',nstep
             print *,'errh2o                     = ',errh2o(indexc)
             print *,'forc_rain                  = ',forc_rain_col(indexc)
             print *,'forc_snow                  = ',forc_snow_col(indexc)
             ...
             print *,'qflx_evap_tot              = ',qflx_evap_tot(indexc)
             print *,'qflx_irrig                 = ',qflx_irrig(indexc)
             print *,'qflx_surf_irrig_col        = ',qflx_surf_irrig_col(indexc)
             print *,'qflx_over_supply_col       = ',qflx_over_supply_col(indexc)
             print *,'qflx_supply                = ',atm2lnd_vars%supply_grc(g)
             print *,'f_grd                      = ',ldomain%f_grd(g)
             print *,'qflx_surf                  = ',qflx_surf(indexc)
             print *,'qflx_h2osfc_surf           = ',qflx_h2osfc_surf(indexc)
             print *,'qflx_qrgwl                 = ',qflx_qrgwl(indexc)
             print *,'qflx_drain                 = ',qflx_drain(indexc)
             print *,'qflx_drain_perched         = ',qflx_drain_perched(indexc)
             print *,'qflx_flood                 = ',qflx_floodc(indexc)
             print *,'qflx_snwcp_ice             = ',qflx_snwcp_ice(indexc)
             print *,'qflx_glcice_melt           = ',qflx_glcice_melt(indexc)
             print *,'qflx_glcice_frz            = ',qflx_glcice_frz(indexc)
             print *,'qflx_lateral               = ',qflx_lateral(indexc)
             print *,'total_plant_stored_h2o_col = ',total_plant_stored_h2o_col(indexc)
             print *,'clm model is stopping'
             stop
       ...
       if ( found ) then
          print *,'WARNING: BalanceCheck: soil balance error (W/m2)'
          print *,'nstep         = ',nstep
          print *,'errsoi_col    = ',errsoi_col(indexc)
          print *,'colum number  = ',col_pp%gridcell(indexc)
