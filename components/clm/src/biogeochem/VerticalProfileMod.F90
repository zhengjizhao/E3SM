...
contains
  ...
  subroutine decomp_vertprofiles(bounds,num_soilc, filter_soilc,num_soilp,filter_soilp, &
    ...
    use clm_varctl  , only : use_vertsoilc, use_dynroot
  ...
  !-----------------------------------------------------------------------
  subroutine decomp_vertprofiles2(bounds,filter_nc, &
       soilstate_vars, canopystate_vars, cnstate_vars)
    !
    ! !DESCRIPTION:
    !  calculate vertical profiles for distributing soil and litter C and N
    ...
    ! !ARGUMENTS:
    type(bounds_type)      , intent(in)    :: bounds
    type(clumpfilter)      , intent(inout) :: filter_nc
    type(soilstate_type)   , intent(in)    :: soilstate_vars
    type(canopystate_type) , intent(in)    :: canopystate_vars
    type(cnstate_type)     , intent(inout) :: cnstate_vars
    !
    ...
    associate(                                                               &
         rootfr               => soilstate_vars%rootfr_patch               , & ! Input:  [real(r8)  (:,:) ]  fraction of roots in each soil layer  (nlevgrnd)
         num_soilp            => filter_nc%num_soilp                       , &
         filter_soilp         => filter_nc%soilp                           , &
         num_soilc            => filter_nc%num_soilc                       , &
         filter_soilc         => filter_nc%soilc                           , &
         altmax_lastyear_indx => canopystate_vars%altmax_lastyear_indx_col , & ! Input:  [integer   (:)   ]  frost table depth (m)
  ...
  end subroutine decomp_vertprofiles2
